...
func buildDeployment(args *AdaptorArgs, eventBus *eventbusv1alpha1.EventBus, log logr.Logger) (*appv1.Deployment, error) {
	...
	if eventBus != nil {
		if eventBus.Status.Config.NATS != nil {
			...
			envVars = append(envVars, corev1.EnvVar{Name: common.EnvVarEventBusType, Value: string(apicommon.EventBusNATS)})
			if natsConf.Auth != nil {
				envVars = append(envVars, corev1.EnvVar{Name: common.EnvVarEventBusAuth, Value: string(*natsConf.Auth)})
				if natsConf.AccessSecret != nil {
				...
				envVars = append(envVars, corev1.EnvVar{Name: common.EnvVarEventBusAuth, Value: string(eventbusv1alpha1.AuthStrategyNone)})
			}
		} else {
